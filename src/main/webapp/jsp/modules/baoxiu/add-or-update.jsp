                    ...
                    <div class="widget-area-2 lochana-box-shadow">
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 级联表的字段 -->
                                    ...
                                    <div class="form-group col-md-6 aaaaaa houqinrenyuan">
                                        <label>后勤人员</label>
                                        <div>
                                            <select style="width: 450px" id="houqinrenyuanSelect" name="houqinrenyuanSelect"
                                                    class="selectpicker form-control"  data-live-search="true"
                                                    title="请选择" data-header="请选择" data-size="5" data-width="650px">
                                            </select>
                                        </div>
                                    </div>
                                    <div class="form-group col-md-6 houqinrenyuan">
                                        <label>后勤人员姓名</label>
                                        <input style="width: 450px" id="houqinrenyuanName" name="houqinrenyuanName" class="form-control"
                                               placeholder="后勤人员姓名" readonly>
                                    </div>
                                    <div class="form-group col-md-6 houqinrenyuan">
                                        <label>后勤人员手机号</label>
                                        <input style="width: 450px" id="houqinrenyuanPhone" name="houqinrenyuanPhone" class="form-control"
                                               placeholder="后勤人员手机号" readonly>
                                    </div>
                                    <div class="form-group col-md-6 houqinrenyuan">
                                        <label>后勤人员头像</label>
                                        <img id="houqinrenyuanPhotoImg" src="" width="100" height="100">
                                    </div>
                            <!-- 当前表的字段 -->
                                    <input id="updateId" name="id" type="hidden">
                                <input id="xueshengId" name="xueshengId" type="hidden">
                                <input id="houqinrenyuanId" name="houqinrenyuanId" type="hidden">
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            if(window.sessionStorage.getItem('role') != '后勤人员'){//当前登录用户不为这个
                if($("#houqinrenyuanId") !=null){
                    var houqinrenyuanId = $("#houqinrenyuanId").val();
                    if(houqinrenyuanId == null || houqinrenyuanId =='' || houqinrenyuanId == 'null'){
                        alert("后勤人员不能为空");
                        return;
                    }
                }
    ...
    // 查询列表
        ...
        function houqinrenyuanSelect() {
            //填充下拉框选项
            http("houqinrenyuan/page?page=1&limit=100&sort=&order=", "GET", {}, (res) => {
                if(res.code == 0){
                    houqinrenyuanOptions = res.data.list;
                }
            });
        }

        function houqinrenyuanSelectOne(id) {
            http("houqinrenyuan/info/"+id, "GET", {}, (res) => {
                if(res.code == 0){
                ruleForm = res.data;
                houqinrenyuanShowImg();
                houqinrenyuanShowVideo();
                houqinrenyuanDataBind();
            }
        });
        }
    ...
    <!-- 初始化当前表的下拉框 -->
        ...
        function initializationhouqinrenyuanSelect() {
            var houqinrenyuanSelect = document.getElementById('houqinrenyuanSelect');
            if(houqinrenyuanSelect != null && houqinrenyuanOptions != null  && houqinrenyuanOptions.length > 0 ) {
                for (var i = 0; i < houqinrenyuanOptions.length; i++) {
                        houqinrenyuanSelect.add(new Option(houqinrenyuanOptions[i].houqinrenyuanName, houqinrenyuanOptions[i].id));
                }

                $("#houqinrenyuanSelect").change(function(e) {
                        houqinrenyuanSelectOne(e.target.value);
                });
            }
    ...
    function setSelectOption() {
        ...
        <!--  级联表的下拉框回显  -->
            ...
            if(houqinrenyuanSelect != null && houqinrenyuanOptions != null  && houqinrenyuanOptions.length > 0 ) {
                for (var i = 0; i < houqinrenyuanOptions.length; i++) {
                    if (houqinrenyuanOptions[i].id == ruleForm.houqinrenyuanId) {//下拉框value对比,如果一致就赋值汉字
                        houqinrenyuanSelect.options[i+1].selected = true;
                        $("#houqinrenyuanSelect" ).selectpicker('refresh');
                    }
                }
            }
    }
    ...
    function houqinrenyuanDataBind(){

                    <!-- 把id赋值给当前表的id-->
        $("#houqinrenyuanId").val(ruleForm.id);

        $("#gongsiValue").val(ruleForm.gongsiValue);
        $("#houqinrenyuanName").val(ruleForm.houqinrenyuanName);
        $("#houqinrenyuanPhone").val(ruleForm.houqinrenyuanPhone);
        $("#houqinrenyuanIdNumber").val(ruleForm.houqinrenyuanIdNumber);
        $("#houqinrenyuanEmail").val(ruleForm.houqinrenyuanEmail);
    }
    ...
    function houqinrenyuanShowImg() {
        $("#houqinrenyuanPhotoImg").attr("src",ruleForm.houqinrenyuanPhoto);
    }
    ...
    function houqinrenyuanShowVideo() {
        $("#houqinrenyuanPhotoV").attr("src",ruleForm.houqinrenyuanPhoto);
    }
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            ...
            initializationhouqinrenyuanSelect();
