...
<body>
    <div class="modal fade" id="plusOrReduceHaocaiKucunNumberModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <%-- 模态框标题--%>
                <div class="modal-header">
                    <h5 class="modal-title" id="plusOrReduceHaocaiKucunNumberTitle">增加或减少库存</h5>
                ...
                <div class="modal-body">
                    数量:<input type="number" id="plusOrReduceHaocaiKucunNumber" style="width: 300px;" class="form-control form-control-sm"
                    ...
                    <input type="hidden" id="haocaiId">
                    <input type="hidden" id="hideflag">
                    <input type="hidden" id="thisHaocaiKucunNumber">
                </div>
                <%-- 模态框按钮 --%>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                    <button type="button" onclick="submitPlusOrReduceHaocaiKucunNumber()" class="btn btn-primary">提交</button>
                </div>
            </div>
        </div>
    </div>
...
<!--/Pre Loader -->
    ...
    <!-- 本表的查询条件 -->
        ...
        function setDataRow(item, number) {
            ...
            //耗材库存数量
            var haocaiKucunNumberCell = document.createElement('td');
            haocaiKucunNumberCell.innerHTML = item.haocaiKucunNumber;
            row.appendChild(haocaiKucunNumberCell);
            ...
            var plusAttr = "plusHaocaiKucunNumber(" + item.id+','+item.haocaiKucunNumber+")";
            ...
            var reduceAttr = "reduceHaocaiKucunNumber(" + item.id+','+item.haocaiKucunNumber+')';
        ...
        function remove(id) {
            ...
            if (mymessage == true) {
                ...
                httpJson("haocai/delete", "POST", paramArray, (res) => {
                    if(res.code == 0){
                        getDataList();
                        alert('删除成功');
                    }
        ...
        //添加
        function add() {
            window.sessionStorage.setItem("addhaocai", "addhaocai");
            window.location.href = "add-or-update.jsp"
        }
    ...
    //查询当前表搜索条件所有列表
        ...
        //增加库存
        function plusHaocaiKucunNumber(thisId,haocaiKucunNumber) {
            if(thisId == null || thisId=="" || thisId=="null"){
                alert("未知错误,请联系管理员处理");
                return;
            }
            $("#plusOrReduceHaocaiKucunNumberTitle").val("增加库存");
            $("#thisHaocaiKucunNumber").val(haocaiKucunNumber);
            $("#hideflag").val("+");
            $("#haocaiId").val(thisId);
            id=thisId;
            $('#plusOrReduceHaocaiKucunNumberModal').modal('show');
        }

        //减少库存
        function reduceHaocaiKucunNumber(thisId,haocaiKucunNumber) {
            if(thisId == null || thisId=="" || thisId=="null"){
                alert("未知错误,请联系管理员处理");
                return;
            }
            $("#plusOrReduceHaocaiKucunNumberTitle").val("减少库存");
            $("#thisHaocaiKucunNumber").val(haocaiKucunNumber);
            $("#hideflag").val("-");
            $("#haocaiId").val(thisId);
            id=thisId;
        ...
        $('#plusOrReduceHaocaiKucunNumberModal').on('show.bs.modal', function () {
            //清理表格内容,防止重复回显
            $("#plusOrReduceHaocaiKucunNumber").val(0);
            if(id ==null){
                alert("获取当前列id错误,请联系管理员处理");
                $('#plusOrReduceHaocaiKucunNumberModal').modal('hide');
                $("#plusOrReduceHaocaiKucunNumberTitle").val("");
                $("#thisHaocaiKucunNumber").val("");
                $("#hideflag").val("");
                $("#haocaiId").val("");
                getDataList();
                return;
        ...
        //模态框关闭的时候会执行这个里面的东西
        $('#plusOrReduceHaocaiKucunNumberModal').on('hide.bs.modal', function () {
            id=null;
            $("#plusOrReduceHaocaiKucunNumber").val(0);
            $("#plusOrReduceHaocaiKucunNumberTitle").val("");
            $("#thisHaocaiKucunNumber").val("");
        ...
        function submitPlusOrReduceHaocaiKucunNumber() {
            var id = $("#haocaiId").val();//当前数据id
            var hideflag = $("#hideflag").val();//是加还是减
            var thisHaocaiKucunNumber = $("#thisHaocaiKucunNumber").val();//原先的数量
            var plusOrReduceHaocaiKucunNumber = $("#plusOrReduceHaocaiKucunNumber").val();//增加或者减少的数量
            var sumNumber=0;
            var msg;
            if(hideflag == "+"){
                sumNumber =parseInt(thisHaocaiKucunNumber) + parseInt(plusOrReduceHaocaiKucunNumber);
                msg = "增加库存成功";
            }else if(hideflag == "-"){
                sumNumber =parseInt(thisHaocaiKucunNumber) - parseInt(plusOrReduceHaocaiKucunNumber);
                msg = "减少库存成功";
            ...
            let data = {"id":id,"haocaiKucunNumber":sumNumber};
            httpJson("haocai/update", "POST", data, (res) => {
                if(res.code == 0){
                    alert(msg);
                    getDataList();
                }else{
